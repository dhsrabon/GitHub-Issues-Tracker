...
function loadIssues(filter = "all") {
  ...
  setTimeout(() => {
    ...
    if (filtered.length === 0) {
      container.innerHTML = `
        <div class="col-span-full bg-white rounded-xl shadow border border-gray-200 p-8 text-center text-gray-500">
          No issues found.
        </div>
      `;
    } else {
      filtered.slice(0, 12).forEach(issue => renderIssue(issue));
    }
...
function renderIssue(issue) {
  const borderColor = issue.status === "open" ? "bg-green-500" : "bg-purple-500";

  const labels = issue.labels.map(label => `
    <span class="text-xs px-2 py-1 rounded-full bg-orange-100 text-orange-600 break-words">
      ${label.toUpperCase()}
    </span>
  `).join("");
  ...
  card.className = "bg-white rounded-xl shadow border border-gray-200 overflow-hidden hover:shadow-md transition cursor-pointer";
  card.innerHTML = `
    <div class="h-1 ${borderColor}"></div>

    <div class="p-5">
      <div class="flex justify-end items-center mb-3">
        <span class="text-xs px-3 py-1 rounded-full ${priorityColor} font-semibold">
          ${issue.priority.toUpperCase()}
        </span>
      </div>

      <h3 class="font-semibold text-gray-800 text-sm mb-2 break-words">
        ${issue.title}
      </h3>

      <p class="text-xs text-gray-500 mb-4 line-clamp-2 break-words">
        ${issue.description}
      </p>
    ...
    <div class="border-t px-5 py-3 text-xs text-gray-500 break-words">
      #${issue.id} by
      <span class="text-blue-600">${issue.author}</span>
      <div class="mt-1">${new Date(issue.createdAt).toLocaleDateString()}</div>
    </div>
  `;
...
function openModal(id) {
  ...
  document.getElementById("modalDesc").innerHTML = `
    ...
    <p><b>Assignee:</b> ${issue.assignee || "None"}</p>
...
document.getElementById("allTab").onclick = () => loadIssues("all");
document.getElementById("openTab").onclick = () => loadIssues("open");
document.getElementById("closedTab").onclick = () => loadIssues("closed");
